...
app.MapGet("/api/config", (BridgeProxyService service) =>
	Results.Json(service.GetConfiguration(), BridgeJsonSerializerContext.Default.BridgeConfigurationView));
app.MapPut("/api/config", async (HttpContext context, BridgeProxyService service, CancellationToken cancellationToken) =>
{
	var update = await JsonSerializer.DeserializeAsync(
		context.Request.Body,
		BridgeJsonSerializerContext.Default.BridgeConfigurationUpdate,
		cancellationToken);
	...
	var updated = service.UpdateConfiguration(update);
